...
export function PreferencesForm({ action, error, footnote, profile, preferences, submitLabel, success }: PreferencesFormProps) {
  ...
  return (
    <Card className="mx-auto max-w-5xl p-6 shadow-glow">
      ...
      {success ? (
        <div className="mb-5 rounded-md border border-success/30 bg-success/10 px-3 py-2 text-sm text-green-200">
          {success}
        </div>
      ) : null}
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            ...
            <label className="block">
              ...
              <select className="form-control" defaultValue={profile?.state ?? preferences?.states?.[0] ?? "GO"} name="state" required>
                {["GO", "DF", "MT", "MS", "SP", "MG", "RJ", "BA", "PR", "RS", "PE", "CE"].map((uf) => (
